...
import {
  IconAlertTriangle, IconCalendarDue, IconCheck, IconChevronDown, IconChevronLeft, IconChevronRight,
  IconCircleCheck, IconClock, IconFolderPlus, IconListTree, IconPaperclip, IconPlus,
  IconRefresh, IconSearch, IconSettings, IconTarget,
} from "@tabler/icons-react";
import { Badge, Button, Card, Col, Form, InputGroup, Modal, ProgressBar, Row, Spinner, Table } from "react-bootstrap";
...
type TaskForm = { title: string; description: string; project: string; parent: string; assignee: string; priority: Priority; status: Status; start_date: string; due_date: string; department: string; project_category: string; admin_notes: string; attachment: File | null };
...
const emptyTask: TaskForm = { title: "", description: "", project: "", parent: "", assignee: "", priority: "MEDIUM", status: "TODO", start_date: "", due_date: "", department: "", project_category: "", admin_notes: "", attachment: null };
...
export default function TaskWorkspace({ employeeMode = false }: { employeeMode?: boolean }) {
  ...
  const load = async () => {
    ...
    try {
      const requests = [fetch(`${projectsApi}?page_size=100`, { headers: headers() }), fetch(`${apiRoot}/tasks/?page_size=100`, { headers: headers() })];
      if (!employeeMode) requests.push(fetch(`${apiRoot}/employees/?page_size=100`, { headers: headers() }));
      ...
      if (!employeeMode && responses[2]?.ok) setEmployees(list<Employee>(await responses[2].json()).filter((item) => item.status === "ACTIVE"));
  ...
  const visibleTasks = useMemo(() => {
    ...
    return tasks.filter((item) => (projectFilter === "ALL" || item.project === projectFilter) && (statusFilter === "ALL" || item.status === statusFilter) && (priorityFilter === "ALL" || item.priority === priorityFilter) && (!needle || [item.title, item.description, item.assignee_name, item.project_name, item.project_key].filter(Boolean).some((value) => value!.toLowerCase().includes(needle))));
  }, [tasks, query, projectFilter, statusFilter, priorityFilter]);
  const rootTasks = visibleTasks.filter((item) => !item.parent);
  const subtasksByParent = useMemo(() => visibleTasks.filter((item) => item.parent).reduce<Record<string, Task[]>>((groups, item) => { if (item.parent) (groups[item.parent] ||= []).push(item); return groups; }, {}), [visibleTasks]);
  ...
  const openTask = (task?: Task, parent?: Task) => {
    ...
    setTaskForm(task ? { title: task.title, description: task.description || "", project: task.project || "", parent: task.parent || "", assignee: task.assignee || "", priority: task.priority, status: task.status, start_date: task.start_date || "", due_date: task.due_date || "", department: task.department || "", project_category: task.project_category || "", admin_notes: task.admin_notes || "", attachment: null } : { ...emptyTask, project: parent?.project || (projectFilter === "ALL" ? "" : projectFilter), parent: parent?.id || "", assignee: !employeeMode ? employees[0]?.id || "" : "", department: parent?.department || "" });
    setTaskModal(true);
  };
  ...
  const saveTask = async (event: React.FormEvent) => {
    ...
    try {
      ...
      Object.entries(taskForm).forEach(([key, value]) => { if (key !== "attachment" && value) payload.append(key, value); });
      payload.set("project", taskForm.project); payload.set("status", taskForm.status); payload.set("priority", taskForm.priority);
      if (!employeeMode) payload.set("assignee", taskForm.assignee);
  ...
  const saveProgress = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(await errorFrom(response));
      setProgressModal(null); setNotice("Progress updated."); await load();
  ...
  return <div className="container-fluid px-0 py-4 task-workspace">
    ...
    <style suppressHydrationWarning>{styles}{heroStyles}{taskTableStyles}{detailStyles}{tableEnhancements}{projectColorStyles}{categoryRemovalStyles}{datePlannerStyles}</style>
  </div>;
}
...
function TaskTableRows({ task, index, subtasksByParent, projectColors, employeeMode, onOpenDetail, onAddSubtask, onEdit, onProgress }: { task: Task; index: number; subtasksByParent: Record<string, Task[]>; projectColors: Record<string, string>; employeeMode: boolean; onOpenDetail: (task: Task) => void; onAddSubtask: (task: Task) => void; onEdit: (task: Task) => void; onProgress: (task: Task) => void }) {
  const renderRows = (item: Task, level = 0, number?: string): React.ReactNode[] => {
    const row = <tr key={item.id} className={`workspace-task-row ${level ? "is-subtask" : ""}`} onClick={() => onOpenDetail(item)} tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpenDetail(item); } }}>
      ...
      {!employeeMode && <td className="py-3" data-label="Assigned to"><div className="d-flex align-items-center gap-2"><img className="task-avatar task-avatar-image" src={item.assignee_avatar_url || getAssetPath("/images/avatar/avatar-fallback.jpg")} alt={item.assignee_name || "Assignee"} /><div><div className="fw-semibold small">{item.assignee_name || "Unassigned"}</div><div className="text-secondary small">{item.department || "No department"}</div></div></div></td>}
      <td className={`py-3 small fw-medium ${item.is_overdue ? "text-danger" : ""}`} data-label="Due date">{date(item.due_date)}</td>
      <td className="py-3" data-label="Priority"><Badge className={`task-priority-chip priority-${item.priority.toLowerCase()}`}>{item.priority_label}</Badge></td>
      <td className="py-3" data-label="Status"><Badge className={`task-status-chip status-${item.is_overdue ? "overdue" : item.status.toLowerCase().replace("_", "-")}`}>{item.is_overdue ? "Overdue" : item.status_label}</Badge></td>
      <td className="px-4 py-3 text-end" data-label="Action"><div className="d-flex justify-content-end gap-2"><Button variant="outline-secondary" size="sm" title="Add subtask" onClick={(event) => { event.stopPropagation(); onAddSubtask(item); }}><IconPlus size={16} /></Button>{employeeMode ? <Button variant="outline-primary" size="sm" disabled={item.status === "CANCELLED"} onClick={(event) => { event.stopPropagation(); onProgress(item); }}>Update</Button> : <Button variant="outline-primary" size="sm" onClick={(event) => { event.stopPropagation(); onEdit(item); }}>Edit</Button>}</div></td>
    </tr>;
